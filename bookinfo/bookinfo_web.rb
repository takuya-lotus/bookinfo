...
# Stringクラスのcontactメソッドを置き換えるバッチ
class String
...
# WEBrickのHTTPServerクラスのサーバーインスタンスを作成する
...
server.mount_proc("/list") { |req, res|
	p req.query
	# 'operation'の入力の後の(.delete, .edit)で処理を分岐する
...
server.mount_proc('/entry') { |req, res|
	# （注意）本来ならここで入力データに危険や不正がないかチェックするが、割愛
	p req.query
...
server.mount_proc("/retrieve") { |req, res|
	...
	if a.empty?
		...
		# 要素があるときは、where句に直す（）現状、項目ごとの完全一致のorだけ）
...
server.mount_proc("/edit") { |req, res|
	...
	page='#{req.query['page']}',pdate='#{req.query['pdate']}'\
